...
public class ZoomImageView extends View {
	...
	private TouchEventCountThread mTouchEventCount = new TouchEventCountThread();
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			if (event.getPointerCount() == 1) {
				...
				if (lastXMove == -1 && lastYMove == -1) { // 如果是第一次拖动
				...
				MLog.d("movedDistanceX::"+movedDistanceX+"&&movedDistanceY"+movedDistanceY);
				// 进行边界检查，不允许将图片拖出边界
				...
				if ((currentStatus == STATUS_ZOOM_OUT && totalRatio < 4 * initRatio)
						|| (currentStatus == STATUS_ZOOM_IN && totalRatio > initRatio*0.5)) {
					scaledRatio = (float) (fingerDis / lastFingerDis);
					totalRatio = totalRatio * scaledRatio;
					if (totalRatio > 4 * initRatio) {
						totalRatio = 4 * initRatio;
					} else if (totalRatio < initRatio*0.5f) {
						totalRatio = initRatio*0.5f;
					}
					// 调用onDraw()方法绘制图片
					invalidate();
		...
		case MotionEvent.ACTION_UP:
			upX = event.getX();
			upY = event.getY();
			disX = Math.abs(downX-upX);
			disY = Math.abs(downY-upY);
			disS = (float) Math.sqrt(disX* disX+disY*disY);
			// 手指离开屏幕时将临时值还原
			lastXMove = -1;
			lastYMove = -1;
			mTouchEventCount.touchCount++;
			MLog.d("mTouchEventCount.touchCount::"+mTouchEventCount.touchCount);
	...
	public class TouchEventHandler extends Handler{
		...
		public void handleMessage(Message msg) {
			...
			if(msg.arg1 == 1 && msg.arg2 < 5){
				performSingleClick();
			}
//			if(msg.arg1 == 2){ //表示是双击事件
//					currentStatus = ZoomImageView.STATUS_ZOOM_OUT;
//					totalRatio = 2;
//					invalidate();
//			}
		}
	...
	public void setOnSingleClickListener (OnSingleClickListener singleClickListener){
		mOnSingleClickListener = singleClickListener;
	}
	
	public boolean performSingleClick() {
        boolean result = false;
        if(mOnSingleClickListener != null) {
        	mOnSingleClickListener.onSingleClick(this);
            result = true;
        }
        return result;
    }
